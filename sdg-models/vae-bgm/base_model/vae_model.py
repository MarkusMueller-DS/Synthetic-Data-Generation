...
class VariationalAutoencoder(torch.nn.Module):
    def __init__(self, params):
        ...
        # print(self.feat_distributions)
        # print(len(self.feat_distributions))
    ...
    def fit_epoch(self, data, optimizer, batch_size=64, device=torch.device("cpu")):
        ...
        for batch in range(n_batches):
            ...
            index_end = min(
                ((batch + 1) * batch_size, cov_train.shape[0])
            )  # Use the min to prevent errors due to samples being smaller than batch_size
        ...
        for epoch in range(train_params["n_epochs"]):
            ...
            if epoch % 1 == 0:
                print(
                    ...
                    epoch,
                    "; train loss = ",
                    "{:.2f}".format(epoch_results["loss_tr"]),
                    "; val loss = ",
                    "{:.2f}".format(epoch_results["loss_va"]),
                    "; ll_cov_tr = ",
                    "{:.2f}".format(np.sum(epoch_results["ll_cov_tr"])),
                    "; ll_cov_va = ",
                    "{:.2f}".format(np.sum(epoch_results["ll_cov_va"])),
                    "; kl_tr = ",
                    "{:.2f}".format(epoch_results["kl_tr"]),
                    "; kl_va = ",
                    "{:.2f}".format(epoch_results["kl_va"]),
                )
