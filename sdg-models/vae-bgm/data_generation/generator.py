...
class Generator(VariationalAutoencoder):
    ...
    def train_latent_generator(self, x):  # Train latent generator using x data as input
        ...
        while (
            not converged and n_try < max_try
        ):  # BGM may not converge: try different times until it converges
            ...
            bgm = BayesianGaussianMixture(
                ...
                mu_latent_param.detach().cpu().numpy()
            )  # Use only mean
            converged = bgm.converged_

        if not converged:
            print(
                "[WARNING] NOT CONVERGED. BGM did not converge after "
                + str(n_try + 1)
                + " attempts"
            )
        else:
            self.bgm = {
                "bgm": bgm,
                "log_var_mean": np.mean(
                    log_var_latent_param.detach().cpu().numpy(), axis=0
                ),
            }  # BGM data to generate patients
    ...
    def sample_space(self, z, mu_sample, log_var_sample, model_type="bgm"):
        ...
        if model_type == "bgm":
            if self.gen_info is None:
                ...
                self.gen_info["z"] = np.concatenate(
                    (self.gen_info["z"], z.detach().cpu().numpy()), axis=0
                )
                self.gen_info["cov_params"] = np.concatenate(
                    (self.gen_info["cov_params"], cov_params), axis=0
                )
                self.gen_info["cov_samples"] = np.concatenate(
                    (self.gen_info["cov_samples"], cov_samples), axis=0
                )
                self.gen_info["latent_params"][0] = np.concatenate(
                    (self.gen_info["latent_params"][0], mu_sample), axis=0
                )
                self.gen_info["latent_params"][1] = np.concatenate(
                    (self.gen_info["latent_params"][1], log_var_sample), axis=0
                )
            ...
            if self.gauss_gen_info is None:
                ...
                # Concatenate new samples
                self.gauss_gen_info["z"] = np.concatenate(
                    (self.gauss_gen_info["z"], z.detach().cpu().numpy()), axis=0
                )
                self.gauss_gen_info["cov_params"] = np.concatenate(
                    (self.gauss_gen_info["cov_params"], cov_params), axis=0
                )
                self.gauss_gen_info["cov_samples"] = np.concatenate(
                    (self.gauss_gen_info["cov_samples"], cov_samples), axis=0
                )
    ...
    def generate(self, n_gen=1000):
        if self.bgm is None:
            print(
                "[WARNING] BGM  is not trained, try calling train_latent_generator before calling generate"
            )
        else:
            ...
            z = self.latent_space.sample_latent(
                [
                    torch.from_numpy(mu_sample).float(),
    ...
    def validate_samples_range(self, data_manager, n_gen, gauss=False):
        if gauss:
            ...
            while self.gauss_gen_info["cov_samples"].shape[0] < n_gen:
                self.generate_gauss(
                    n_gen=n_gen - self.gauss_gen_info["cov_samples"].shape[0]
                )
                self.gauss_gen_info = data_manager.postprocess_gen_data(
                    self.gauss_gen_info
                )
        else:
            # Denormalize generated samples to check data ranges
    ...
    def compare_latent_spaces(self, data, path):
        ...
        if z.shape[0] > 500:
            ...
            z_bgm = np.array(
                z_bgm_pd.sample(n=500, random_state=0).reset_index(drop=True)
            )
            z_gauss_pd = pd.DataFrame(z_gauss)
            z_gauss = np.array(
                z_gauss_pd.sample(n=500, random_state=0).reset_index(drop=True)
            )
        ...
        plt.scatter(
            z_tsne[z.shape[0] : (z.shape[0] + z_bgm.shape[0]) - 1, 0],
            z_tsne[z.shape[0] : (z.shape[0] + z_bgm.shape[0]) - 1, 1],
            alpha=0.3,
        )
        plt.scatter(
            z_tsne[(z.shape[0] + z_bgm.shape[0]) :, 0],
            z_tsne[(z.shape[0] + z_bgm.shape[0]) :, 1],
            alpha=0.3,
        )
