...
parser = argparse.ArgumentParser(
    description="Args for synthetic data generation with vae-bgm"
)
...
def get_best_seed_disc_results(results, clas, mode, args, best_seeds=3):
    ...
    print("param_comb:", args["param_comb"])
    for params in args["param_comb"]:
        print(params)
    for params in args["param_comb"]:
        model_params = str(params["latent_dim"]) + "_" + str(params["hidden_size"])
        print("type resuls:", type(results))
        print("model_params:", model_params)
        seeds_acc = [
            (
                seed,
                seed_results["disc"][mode][clas][0],
                seed_results["disc"][mode][clas][1],
                seed_results["disc"][mode][clas][2],
            )
            for seed, seed_results in results[model_params].items()
        ]
        ...
        if avg_acc < clas_best_results["best_avg_acc"]:
            ...
            for seed in best_seeds_acc:
                best_seeds_f1_score.append(
                    (seed[0], results[model_params][seed[0]]["disc"][mode][clas][5])
                )
            avg_f1_score = sum(ele[1] for ele in best_seeds_f1_score) / len(
                best_seeds_f1_score
            )
...
def show_utility_results(results, best_rf_results, dataset_name, mode, args):
    utility_best_results = {
        "param_comb": best_rf_results["param_comb"],
        "avg_metric": 1,
        "best_seeds_metric": [],
    }
    ...
    for seed in best_seeds:
        ...
        utility_best_results["best_seeds_metric"].append(
            (seed, res_seed["real_real"], res_seed["gen_real"])
        )

    sum_metric = 0.0
    for ele in utility_best_results["best_seeds_metric"]:
        sum_metric += ele[2][1]
    utility_best_results["avg_metric"] = sum_metric / len(
        utility_best_results["best_seeds_metric"]
    )
...
def show_sdv_best_reports(results, best_rf_results, mode):
    ...
    best_seeds = [seed[0] for seed in best_rf_results["best_seeds_acc"]]
    for seed in best_seeds:
        res_seed = results[best_rf_results["param_comb"]][seed]["stats"][
            mode
        ].get_score()
    ...
    for ele in sdv_best_results["best_seeds_score"]:
        sum_metric += ele[1]
    sdv_best_results["avg_score"] = sum_metric / len(
        sdv_best_results["best_seeds_score"]
    )
...
def train(data_manager, params, seed, output_dir, args):
    ...
    model_params = {
        "feat_distributions": data_manager.feat_distributions,
    ...
    if args["gauss"]:
        ...
        # Plot latent spaces
        model.compare_latent_spaces(
            data[0], output_dir + "gauss" + os.sep + model_path + os.sep
        )
        gauss_gen_data = data_manager.save_gauss_data_to_csv(
            output_dir, model_path, model.gauss_gen_info
        )
        return (
            params,
            seed,
...
def evaluate(data_manager, params, seed, args, model="bgm"):
    ...
    comp_gen_df = (
        data_manager.gen_info[p_name][seed]["cov_samples"]
        if model == "bgm"
        else data_manager.gauss_gen_info[p_name][seed]["cov_samples"]
    )
    ...
    for mode in validation:
        if mode == "cov_comp":
            results["disc"][mode] = discriminative_validation(
                imp_real_df, comp_gen_df, data_manager, classifier_list
            )
            results["task"][mode] = utility_validation(
                imp_real_df, comp_gen_df, data_manager, args
            )
            results["stats"][mode] = ctgan_score_validation(
                imp_real_df, comp_gen_df, data_manager.metadata
            )
        else:
...
def eval_model(data_manager, args, output_dir, model="bgm"):
    model_results = Parallel(n_jobs=args["n_threads"], verbose=10)(
        delayed(evaluate)(data_manager, params, seed, args, model)
        for params in args["param_comb"]
    ...
    for dataset_name in args["datasets"]:
        ...
        if args["train"]:
            ...
            data_manager = check_file(
                output_dir + "data_manager.pkl", "Data manager not found"
            )
        ...
        for model in model_results:
            ...
            results = check_file(
                output_dir + model + os.sep + "results.pkl",
            ...
            for mode in validation:
                ...
                for clas in args["classifiers_list"]:
                    ...
                    print(
                        "Average accuracy from best seeds: "
                        + str(best_results["best_avg_acc"])
                    )
                    print(
                        "Average F1 score from best seeds: "
                        + str(best_results["best_avg_f1_score"])
                    )
                    for acc, f1_score in zip(
                        best_results["best_seeds_acc"],
                ...
                if "cov" in mode and best_rf_results is not None:
                    print("\n----UTILITY VALIDATION----")
                    show_utility_results(
                        results, best_rf_results, dataset_name, mode, args
                    )
