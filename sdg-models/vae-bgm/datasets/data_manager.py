...
class DataManager:
    def __init__(
        self, dataset_name, raw_df, processed_df, mapping_info=None, raw_metadata=None
    ):
    ...
    def concat_mask(self):
        ...
        self.feat_distributions.extend(
            [
                ("bernoulli", 1)
                for _ in range(self.mask.shape[1] - len(self.feat_distributions))
            ]
        )

    def split_data_old(self, split=0.2):
        # Split data
        train_data, test_data, train_mask, test_mask = train_test_split(
            self.imp_norm_df, self.mask, test_size=split, random_state=0
        )
        self.model_data = (
    ...
    def split_data(self, split_list):
        df_data = self.imp_norm_df
        df_data["split"] = split_list
    ...
    def mice_imputation(self, data, model="bayesian"):
        ...
        imp = IterativeImputer(
            estimator=clf, verbose=2, max_iter=30, tol=1e-10, imputation_order="roman"
        )
    ...
    def statistics_imputation(self, data, norm):
        ...
        n_columns = (
            data.columns.size
            if data.columns.size < self.columns.size
            else self.columns.size
        )
        for i in range(n_columns):
            ...
            if any(pd.isnull(values)):
                ...
                if (
                    values.dtype in [object, str]
                    or no_nan_values.size <= 2
                    or np.amin(np.equal(np.mod(no_nan_values, 1), 0))
                ):
                ...
                # If raw data has int values take mode normalized
                    ...
                    idx = np.where(self.processed_df.iloc[:, i].values == stats_value)[
                        0
                    ][0]
                    # Find which value is in idx of data.iloc[:, i].values and set this value to stats_value
                    stats_value = values[
                        np.where(values == data.iloc[:, i].values[idx])[0][0]
                    ]
                else:
                    stats_value = no_nan_values.mean()
                imp_data.iloc[:, i] = [
                    stats_value if pd.isnull(x) else x for x in imp_data.iloc[:, i]
                ]
        ...
        for i in range(self.processed_df.shape[1]):
            ...
            if denorm:  # Denormalize
                transformed_df.iloc[:, i] = (
                    df.iloc[:, i] * scale + loc if scale != 0 else df.iloc[:, i] + loc
                ).astype(self.processed_df.iloc[:, i].dtype)
            else:  # Normalize
                transformed_df.iloc[:, i] = (
                    (df.iloc[:, i] - loc) / scale if scale != 0 else df.iloc[:, i] - loc
                )
    ...
    def postprocess_gen_data(self, gen_info):
        ...
        for idx in range(self.columns.size):
            ...
            if idx in self.positive_gaussian_cols:
                ...
                gen_info["cov_params"] = gen_info["cov_params"][
                    denorm_gen_df.iloc[:, idx] >= 0
                ]
                gen_info["latent_params"][0] = gen_info["latent_params"][0][
                    denorm_gen_df.iloc[:, idx] >= 0
                ]
                gen_info["latent_params"][1] = gen_info["latent_params"][1][
                    denorm_gen_df.iloc[:, idx] >= 0
                ]
            ...
            if self.feat_distributions[idx][0] == "gaussian":
                no_nan_values = self.processed_df.iloc[:, idx].values[
                    ~pd.isnull(self.processed_df.iloc[:, idx].values)
                ]
        ...
        if self.mapping_info is not None:
            for key in self.mapping_info.keys():
                for idx, val in enumerate(self.mapping_info[key]):
                    ...
                    transf_gen_raw_data[key] = transf_gen_raw_data[key].replace(
                        idx, val
                    )
        transf_gen_raw_data.to_csv(
            path + "bgm" + os.sep + model_path + os.sep + "raw_gen_data.csv",
            index=False,
        )
        gen_data = gen_info["cov_samples"].iloc[:, : self.processed_df.shape[1]]

        if self.generate_mask:
            gen_mask = gen_info["raw_cov_samples"].iloc[
                :, -self.processed_df.shape[1] :
            ]
            gen_mask = gen_mask.set_axis(self.columns, axis=1)
            gen_mask.to_csv(
                path + "bgm" + os.sep + model_path + os.sep + "gen_mask.csv",
                index=False,
            )
            ...
            gen_nan_raw_data.to_csv(
                path + "bgm" + os.sep + model_path + os.sep + "raw_nan_gen_data.csv",
                index=False,
            )
    ...
    def save_gauss_data_to_csv(self, path, model_path, gauss_gen_info):
        # Separate raw_cov_samples into data and mask
        gen_raw_data = gauss_gen_info["raw_cov_samples"].iloc[
            :, : self.processed_df.shape[1]
        ]
        # Before saving data to csv, we need to transform numerical values to categorical if necessary
        # Iter keys from self.str_mapping and transform gen values
        ...
        transf_gen_raw_data.to_csv(
            path + "gauss" + os.sep + model_path + os.sep + "raw_gauss_gen_data.csv",
            index=False,
        )
        gen_data = gauss_gen_info["cov_samples"].iloc[:, : self.processed_df.shape[1]]

        if self.generate_mask:
            gen_mask = gauss_gen_info["raw_cov_samples"].iloc[
                :, -self.processed_df.shape[1] :
            ]
            gen_mask = gen_mask.set_axis(self.columns, axis=1)
            gen_mask.to_csv(
                path + "gauss" + os.sep + model_path + os.sep + "gauss_gen_mask.csv",
                index=False,
            )
