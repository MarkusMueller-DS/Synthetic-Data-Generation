...
def bern_conf_interval(n, mean, acc=True):
    # Confidence interval
    ci_bot, ci_top = proportion_confint(
        count=mean * n, nobs=n, alpha=0.1, method="beta"
    )
    if mean < 0.5 and acc:
        ci_bot_2 = 1 - ci_top
        ci_top = 1 - ci_bot
...
def define_classifiers(disc_list):
    ...
    for clas in disc_list:
        if clas == "RF":
            classifiers[clas] = RandomForestClassifier(
                n_estimators=1000, random_state=0, n_jobs=3
            )
        else:
...
def discriminative_validation(
    real_df, synthetic_df, data_manager, classifier_list, split=0.8
):
    ...
    real_df["Class"] = np.zeros(real_df.shape[0]).astype(int)
    synthetic_df["Class"] = np.ones(synthetic_df.shape[0]).astype(int)
    mixed_data = [real_df, synthetic_df]
    mixed_data = (
        pd.concat(mixed_data, axis=0)
        .sample(frac=1, random_state=0)
        .reset_index(drop=True)
    )

    # Preprocess data
    x_train, y_train, x_test, y_test = preprocess_ml_data(
        mixed_data, data_manager, split
    )
    ...
    for clas in classifiers_names:
        results[clas] = classification_task(
            classifiers[clas], x_train, y_train, x_test, y_test
        )
...
def survival_analysis(real_df, synthetic_df, data_manager):
    ...
    concat_df = pd.concat(
        [real_df, synthetic_df], axis=0
    )  # No shuffle to keep the order
    real_x, real_y, syn_x, syn_y = preprocess_sa_data(
        concat_df, real_df.shape[0], data_manager
    )

    # Split data
    tr_real_x, te_real_x, tr_real_y, te_real_y = train_test_split(
        real_x, real_y, test_size=0.2, random_state=0
    )
    tr_real_x, va_real_x, tr_real_y, va_real_y = train_test_split(
        tr_real_x, tr_real_y, test_size=0.2, random_state=0
    )
    tr_syn_x, te_syn_x, tr_syn_y, te_syn_y = train_test_split(
        syn_x, syn_y, test_size=0.2, random_state=0
    )
    tr_syn_x, va_syn_x, tr_syn_y, va_syn_y = train_test_split(
        tr_syn_x, tr_syn_y, test_size=0.2, random_state=0
    )
    ...
    for mode in modes:
        if mode == "real_real":
            ...
            y_train = (
                tr_real_y.iloc[:, -2].values.astype("float32"),
                tr_real_y.iloc[:, -1].values.astype("float32"),
            )
            x_val = va_real_x.to_numpy().astype("float32")
            y_val = (
                va_real_y.iloc[:, -2].values.astype("float32"),
                va_real_y.iloc[:, -1].values.astype("float32"),
            )
            val = (x_val, y_val)
            x_test = te_real_x.to_numpy().astype("float32")
            y_test = (
                te_real_y.iloc[:, -2].values.astype("float32"),
                te_real_y.iloc[:, -1].values.astype("float32"),
            )
        elif mode == "gen_real":
            # 2. Train with syn data and test with real data
            x_train = tr_syn_x.to_numpy().astype("float32")
            y_train = (
                tr_syn_y.iloc[:, -2].values.astype("float32"),
                tr_syn_y.iloc[:, -1].values.astype("float32"),
            )
            x_val = va_syn_x.to_numpy().astype("float32")
            y_val = (
                va_syn_y.iloc[:, -2].values.astype("float32"),
                va_syn_y.iloc[:, -1].values.astype("float32"),
            )
            val = (x_val, y_val)
            x_test = te_real_x.to_numpy().astype("float32")
            y_test = (
        ...
        optimizer = tt.optim.AdamWR(
            decoupled_weight_decay=0.01, cycle_eta_multiplier=0.8, cycle_multiplier=2
        )
...
def preprocess_ml_data(data, data_manager, split, target="Class"):
    # Data imputation
...
def classification_task(classifier, x_train, y_train, x_test, y_test):
    ...
    prec = (
        np.round(precision_score(y_test, predictions, zero_division=0), 4)
...
def ml_analysis(real_df, synthetic_df, data_manager, args, task=None, split=0.8):
    ...
    tr_real, te_real, tr_syn, te_syn = train_test_split(
        real_df, synthetic_df, test_size=0.2, random_state=0
    )
    tr_real_x = tr_real.drop(columns=["Class"])
    tr_real_y = tr_real["Class"]
    te_real_x = te_real.drop(columns=["Class"])
    te_real_y = te_real["Class"]
    tr_syn_x = tr_syn.drop(columns=["Class"])
    tr_syn_y = tr_syn["Class"]
    ...
    for i, mode in enumerate(modes):
        ...
        if data_manager.dataset_name in args["cl_datasets"] or task == "cl":
            classifier = RandomForestClassifier(
                n_estimators=10000, random_state=0, n_jobs=3, max_depth=3
            )
            results[mode] = classification_task(
