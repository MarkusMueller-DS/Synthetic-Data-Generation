...
def preprocess_data(dataset_name, args):
    ...
    elif dataset_name == "cc-fraud":
        data = process_cc_fraud(dataset_name)
    ...
    for dataset_name in args["datasets"]:
        if "sota" in task:
            for model in args["models"]:
                os.makedirs(
                    args["sota_output_dir"] + dataset_name + os.sep + model + os.sep,
                    exist_ok=True,
                )
        else:
...
def run_args(dataset):
    ...
    # set amount of synthetic samples to generate
    if args["datasets"] == ["adult"]:
        args["n_gen"] = 33758
    if args["datasets"] == ["yeast"]:
        args["n_gen"] = 658
    if args["datasets"] == ["cc-fraud"]:
        # args["n_gen"] = 77224  # for 1% class imbalance
        args["n_gen"] = 14184  # for 5% class imbalance
    ...
    args["cl_datasets"] = ["cc-fraud-5"]

    # SOTA models
    args["sota_output_dir"] = (
        args["abs_path"] + "data_generation" + os.sep + "output_sota" + os.sep
    )
    model_name = "ctgan"
    ...
    args["train_vae"] = True
    args["output_dir"] = (
        args["abs_path"] + "data_generation" + os.sep + "output_generator" + os.sep
    )
